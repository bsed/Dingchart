...
/// </summary>
public partial class EditBZ : Window {
    public EditBZ() {
        InitializeComponent();
    }
    //变量定义
    ...
    private void Window_Loaded(object sender, RoutedEventArgs e) {
        try {
            ...
            if (txt != null) {
                txt.MaxLength = 10;
                if (ClientuserId != 0) {
                    ContactsTable model = ContactsServices.getInstance().FindContactsById(ClientuserId);
                    if (model != null) {
                        txt.Text = model.alias;
                    }

                }
            }
        } catch (Exception ex) {
            Log.Error(typeof(EditBZ), ex);
        }
    }
    ...
    void btnOk_Click(object sender, RoutedEventArgs e) {
        try {
            ContactsApi.changeAlias(ClientuserId, txt.Text);
