...
public class GroupMemberAddedMessage : Message {
    ...
    public override MsgType getType() {
        return MsgType.GroupMemberAdded;
    }
    ...
    public override void parse(MsgType type, SendMessage sendMessage) {
        try {
            ...
            this.setChannel(json.GetValue("channel").ToStr());
            ...
            List<GroupMemberDetails> jsonArray = JsonConvert.DeserializeObject<List<GroupMemberDetails>>(json["members"].ToString(), new convertor<GroupMemberDetails>());
            this.members = jsonArray;
        } catch (Exception e) {
            Log.Error(typeof(GroupMemberAddedMessage), e);
