...
/// <summary> 汉字编码工具类(可以获取汉字的拼音缩写、五笔编码缩写)
/// </summary>
public class HanBmHelper {
    #region 变量
        ...
        get {
            if (_reader == null) {
                Stream stream = Assembly.GetExecutingAssembly().
                                GetManifestResourceStream(typeof(HanBmHelper).Namespace + ".HanBmHelper.xml");
    ...
    /// <summary> XML文件中数据
    /// </summary>
    private static string[] _xmlData = null;
    ...
    static string[] _saCode;

    private static string[] SaCode {
        get {
            if (_saCode == null) {
                _saCode =
                new string[] {
    ...
    /// <summary> 获取汉字的全拼音
    /// </summary>
    /// <param name="han">传汉字的字符串</param>
    /// <returns>汉字的字符串的拼音</returns>
    public static string GetPyqp(string han) {

        byte[] b = new byte[2];

        string s = string.Empty;

        char[] c = han.ToCharArray();
    ...
    /// <summary> 获得拼音码
    /// </summary>
    /// <param name="han">汉字</param>
    /// <returns>汉语拼音缩写</returns>
    public static string GetPyBm(string han) {
        try {
            return SearchWord(han, 0, wbCodeStation);
        } catch(Exception e) {
            Log.Error(typeof(HanBmHelper), e);
            return string.Empty;
        }
    }
    /// <summary> 获得五笔码
    /// </summary>
    /// <param name="han">汉字</param>
    /// <returns>汉语五笔缩写</returns>
    public static string GetWbBm(string han) {
        try {
            return SearchWord(han, wbCodeStation, outStation);
        } catch(Exception e) {
            Log.Error(typeof(HanBmHelper), e);
            return string.Empty;
        }
